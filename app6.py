...
def get_ollama_models():
    ...
    and filters out any model names containing '/' (Hugging Face style IDs).
    """
    try:
        ...
        if result.returncode == 0:
            ...
            for line in lines[1:]:
                ...
                if tokens:
                    model_name = tokens[0]
                    # Filter out Hugging Face style names (which typically include '/')
                    if "/" in model_name:
                        continue
...
# Define a custom prompt template instructing the assistant to return full text excerpts.
